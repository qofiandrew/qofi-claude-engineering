...
#
# Aliases:
#   swarm-<name>      attach (or launch-then-attach) that specific swarm
#                     (one alias per row in swarm.conf, generated at source)
#   swarm-attach      same, but takes <name> as an argument
...
# Per-swarm aliases generated from swarm.conf — one `swarm-<name>` alias for
# every row. So `swarm-reserve-backend-2` (matching the actual session name
# `swarm-reserve-backend-2`) attaches that swarm, launching it first if
# needed. Re-source this file after `swarm-add.sh` to pick up new swarms.
