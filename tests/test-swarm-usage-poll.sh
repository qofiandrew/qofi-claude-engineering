...
assert_eq 0 "$rc" "40%/30% under default 85% -> OK (exit 0)"
if printf '%s' "$OUT" | grep -q 'OK'; then ok "labels OK"; else bad "should label OK"; fi

# ---------------------------------------------------------------------------
echo "=== 2) NEAR: 5h window crosses threshold -> exit 10 ==="
poll_with '{"five_hour":{"used_pct":88,"reset_hint":"resets 11pm"},"weekly":{"used_pct":20}}'
assert_eq 10 "$rc" "5h=88% >= 85% -> NEAR-LIMIT (exit 10)"
...
poll_with '{"five_hour":{"used_pct":10},"weekly":{"used_pct":90}}'
assert_eq 10 "$rc" "weekly=90% >= 85% -> NEAR-LIMIT (exit 10), 5h low"

echo "=== 2c) boundary: exactly at threshold counts as NEAR ==="
poll_with '{"five_hour":{"used_pct":85},"weekly":{"used_pct":0}}'
assert_eq 10 "$rc" "5h=85% == threshold -> NEAR (>= is inclusive)"

echo "=== 2d) just under threshold stays OK ==="
poll_with '{"five_hour":{"used_pct":84},"weekly":{"used_pct":84}}'
assert_eq 0 "$rc" "84%/84% just under 85% -> OK"
...
poll_with '{"5h":{"pct":88},"week":{"percent":10}}'
assert_eq 10 "$rc" "alias keys (5h.pct / week.percent) parse -> NEAR"
# Scalar form: bare percent under the *_pct key.
poll_with '{"five_hour_pct":92}'
assert_eq 10 "$rc" "scalar five_hour_pct=92 -> NEAR"
...
OUT="$(SWARM_USAGE_PROBE="printf '%s' '{\"five_hour\":{\"used_pct\":88}}'" bash "$POLL" --quiet 2>/dev/null)"; rc=$?
...
OUT="$(SWARM_USAGE_PROBE="printf '%s' '{\"five_hour\":{\"used_pct\":88},\"account\":\"max-b\"}'" bash "$POLL" --json 2>/dev/null)"; rc=$?
