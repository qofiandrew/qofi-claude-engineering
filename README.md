...
| `swarm-login-relay.sh [--force] [<swarm>]` | User-assisted re-auth. Sends `/login` to the swarm's pane (default `qofi-product`), scrapes the OAuth URL from the pane, posts it to that swarm's Discord channel, waits for you to authenticate in the browser (default 15 min), sends Enter to resume, then verifies via `swarm-auth-probe.sh` and maps to the credswap exit contract (0 good / 7 ring-exhausted). One pane is enough: the default account is shared keychain state, so one re-auth covers the fleet. Refuses mid-turn panes without `--force`; single-instance (mkdir lock); every pane detector is freshness-gated against a pre-`/login` baseline so stale pane content can't drive the flow; Escapes out of the login UI on every failure path; re-checks the fleet's clean boundary before handing back to rotate's relaunch. **Rotation wiring:** `export SWARM_CREDSWAP_CMD="$SWARM_HOME/bin/swarm-login-relay.sh"` — no `"$1"` suffix (the account CHOICE happens in your browser, not in the handle). | Re-auth the shared credential remotely — standalone, or as rotation's credential-swap step when blobs can't be provisioned. |
