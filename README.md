...
| `swarm-login-relay.sh [--force] [--dedicated] [<swarm>]` | User-assisted re-auth. Sends `/login` to a pane, scrapes the OAuth URL, posts it to the swarm's Discord channel (default `qofi-product`), waits for you to authenticate in the browser (default 15 min), sends Enter to resume, then verifies via `swarm-auth-probe.sh` and maps to the credswap exit contract (0 good / 7 ring-exhausted). One login is enough: the default account is shared keychain state. **Default mode** targets the swarm's CTO pane: refuses mid-turn panes without `--force`, re-checks the fleet's clean boundary before handing back to rotate's relaunch. **`--dedicated` mode** (the no-restart model) runs `/login` in an isolated throwaway session (`swarm-login-probe`) instead — no CTO pane is touched, so both guards are skipped. Single-instance (mkdir lock); freshness-gated detectors; Escapes out of the login UI on every failure path. | Re-auth the shared credential remotely — standalone, via `swarm-reauth.sh` (no-restart), or as legacy rotation's credential-swap step. |
| `swarm-reauth.sh [--dry-run\|--next\|--force]` | The **no-restart rotation actuator** (wires into the tick's `SWARM_ROTATE_CMD`). On a NEAR/AT verdict it runs the login relay `--dedicated`: `/login` in the isolated session, URL to the product channel, browser picks the account, **nothing restarts** — no checkpoint, no clean-boundary guard, no account ring. On success it recycles the `/usage` probe session so the next poll reads the new account. Exit: 0 re-authed / 6 ring-exhausted (relay said authed-but-capped) / 5 not completed / 2 config. | Rotation without losing any lead's in-RAM state. Pair with `swarm-reauth-verify.sh`. |
